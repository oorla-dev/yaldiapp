document.addEventListener("DOMContentLoaded", () => {
  ...
  const albums = [
     {
      id: 1, title: "YALDHI", year: "2025", image: "images/yaldhi.png", tracks: [
        ...
        { id: 107, title: "Brothers Of Aldi", duration: "4:15", audioSrc: "audio/yaldhi-praise-god.mp3" }, // Es: 'audio' deve essere una cartella allo stesso livello di index.html
        { id: 108, title: "Fuck Gianvoit", duration: "2:00", audioSrc: "YALDHI/fg.mp3" }
      ],
    },
    {
      id: 2, title: "DONDA 2", year: "2022", image: "images/donda2.jpg", tracks: [
     ...
     {
      id: 3, title: "BULDY", year: "2025", image: "images/buldy.jpg", tracks: [
        { id: 301, title: "First Time", duration: "3:33", audioSrc: "audio/buldy-first-time.mp3" }
      ],
    },
    {
      id: 4, title: "JL 2", year: "2025", image: "images/jl2.jpg", tracks: [
  ...
  const products = [
    { id: "AY-01", name: "YALDHI CD", image: "images/yaldhicd.png", price: "€30.00", category: "music" }, { id: "FM-104", name: "FREE MARINAIO TEE", image: "images/fm.jpg", price: "€80.00", category: "apparel" }, { id: "YS-22", name: "YALDHI HOODIE", image: "images/hoodie.jpg", price: "€120.00", category: "apparel" }, { id: "BL-05", name: "BULDY VINYL", image: "images/buldy.jpg", price: "€35.00", category: "music" },
  ];
  const events = [
     { title: "Y$Ø", subtitle: "YALDHI", desc: "LISTENING EXPERIENCE", location: "PERUGIA", venue: "DISCALDI ARENA", date: "1 4 25" }, { title: "YE", subtitle: "DONDA 2", desc: "LISTENING EXPERIENCE", location: "PESARO", venue: "DISCALDI ARENA", date: "4 9 25" },
  ];
  ...
  function setupNavigation() {
    ...
    navItems.forEach((item) => {
      item.addEventListener("click", function () {
        ...
        if (isPlaying) { audioPlayer.pause(); } // Pausa audio al cambio vista
        if(currentView === 'album-detail-view') { updateTrackListUI(null); } // Toglie highlight
  ...
  function renderAlbumGrid() {
    ...
    if (!albumGrid) return; albumGrid.innerHTML = "";
    albums.forEach((album) => {
      const albumElement = document.createElement("div"); albumElement.className = "album-item";
      let imageType = "image/jpeg";
      if (album.image.endsWith(".png")) imageType = "image/png"; else if (album.image.endsWith(".svg")) imageType = "image/svg+xml"; else if (album.image.endsWith(".webp")) imageType = "image/webp";
  ...
  function showAlbumDetail(album) {
    ...
    if (!detailView) { console.error("Vista dettaglio album non trovata"); return; }
    ...
    if (albumDetailSource && albumDetailImg) {
        let imageType = "image/jpeg";
        if (album.image.endsWith(".png")) imageType = "image/png"; else if (album.image.endsWith(".svg")) imageType = "image/svg+xml"; else if (album.image.endsWith(".webp")) imageType = "image/webp";
        albumDetailSource.srcset = album.image; albumDetailSource.type = imageType;
        albumDetailImg.src = album.image; albumDetailImg.alt = `${album.title} album cover`;
    }

    // Render tracks list (SENZA audio incorporati)
    const tracksList = detailView.querySelector(".tracks-list");
    if (!tracksList) { console.error("Contenitore lista tracce non trovato"); return; }
    ...
    if (backButton) {
        ...
        newBackButton.addEventListener("click", () => {
            ...
            document.querySelectorAll('#main-view .nav-item').forEach(navItem => { // Aggiorna nav bar main view
  ...
  function playTrack(track, album) {
    if (!track || !album) { console.error("playTrack chiamata senza traccia o album valido."); return; }
    ...
    if (!track.audioSrc || typeof track.audioSrc !== 'string' || track.audioSrc.trim() === '') {
        console.error(`Errore: Percorso audio non valido per la traccia "${track.title}" (ID: ${track.id}). Ricevuto:`, track.audioSrc);
    ...
    document.querySelectorAll(".player-bar").forEach(playerBar => {
        ...
        const titleEl = playerBar.querySelector(".track-title");
        const albumEl = playerBar.querySelector(".track-album");
        if (img) { img.src = currentAlbum.image || 'placeholder.svg'; img.alt = `Now playing: ${track.title}`; }
  ...
  function playNextTrack() {
    ...
    if (!currentAlbum || !currentTrack) { console.log("Nessuna traccia corrente per andare alla successiva."); return; }
  ...
  function playPrevTrack() {
    ...
    if (!currentAlbum || !currentTrack) { console.log("Nessuna traccia corrente per andare alla precedente."); return; }
    const currentIndex = findTrackIndex(currentTrack, currentAlbum);
    if (currentIndex > 0) {
