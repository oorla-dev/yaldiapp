document.addEventListener("DOMContentLoaded", () => {
  ...
  // Expanded Player References
const expandedPlayer = document.getElementById('expanded-player');
const nowPlaying = document.querySelector('.player-bar');
...
const nextTrackCover = document.querySelector('.next-cover');
...
let isShuffle = false;
let loopMode = 0; // 0: no loop, 1: loop track, 2: loop all

// --- Dati esempio della traccia corrente
let currentTrack = {
...
// --- Dati esempio della prossima traccia
let nextTrack = {
...
function playTrack(track) {
  ...
  expandedArtistName.textContent = track.artist;
  audio.play();
  playBtn.innerHTML = `<i class="fas fa-pause"></i>`;
}

// Clicca su "prossima traccia"
document.getElementById('next-track-info').addEventListener('click', () => {
  playTrack(nextTrack);
});

// Aggiorna UI del player espanso
function updateExpandedPlayer() {
  expandedCover.src = currentTrack.cover;
  expandedTrackTitle.textContent = currentTrack.title;
  expandedArtistName.textContent = currentTrack.artist;

  nextTrackCover.src = nextTrack.cover;
