document.addEventListener("DOMContentLoaded", () => {
  ...
  function init() {
    ...
    // Imposta i tipi MIME supportati per l'audio
  ...
  function playTrack(track, album) {
    ...
    if (!track.audioSrc || typeof track.audioSrc !== "string" || track.audioSrc.trim() === "") {
      ...
      handleErrorEvent(new Error("Percorso audio non valido")) // Simula errore per resettare UI
    ...
    if (!audioPath.startsWith("YALDHI/")) {
      console.warn(`Percorso audio non inizia con YALDHI/: ${audioPath}. Tentativo di correzione automatica.`)
      audioPath = "YALDHI/" + audioPath.split("/").pop()
      console.log(`Percorso corretto: ${audioPath}`)
    }

    // Determina il tipo MIME in base all'estensione del file
    let mimeType = "audio/mpeg" // Default per MP3
    ...
    // Imposta la sorgente audio con il tipo MIME corretto
    try {
      // Svuota le sorgenti esistenti
      while (audioPlayer.firstChild) {
        audioPlayer.removeChild(audioPlayer.firstChild)
      }

      // Crea un nuovo elemento source con il percorso e tipo MIME corretti
      const source = document.createElement("source")
      source.src = audioPath
      source.type = mimeType
      audioPlayer.appendChild(source)

      // Aggiunge un messaggio di fallback
      const fallbackText = document.createTextNode("Il tuo browser non supporta l'elemento audio.")
      audioPlayer.appendChild(fallbackText)

      console.log(`Audio source impostata: ${audioPath} (${mimeType})`)
    } catch (e) {
      console.error("Errore durante l'impostazione della sorgente audio:", e)
    }
    ...
    // Aggiungi un breve ritardo prima di riprodurre per dare tempo al browser di caricare l'audio
    setTimeout(() => {
